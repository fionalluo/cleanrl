...
for ((config_idx=0; config_idx<$NUM_CONFIGS; config_idx++)); do
  ...
  for ((seed_idx=0; seed_idx<$NUM_SEEDS; seed_idx++)); do
    ...
    HELPER_SCRIPT="ts_job_${config_idx}_${seed_idx}.sh"
...
timeout 24h python3 -u thesis/teacher_student/train.py \\
  --configs "\$CONFIG" \\
  --seed "\$SEED"
...
EOF
    ...
    # Optionally, delete the helper script after submitting the job
    rm $HELPER_SCRIPT
  done
done
